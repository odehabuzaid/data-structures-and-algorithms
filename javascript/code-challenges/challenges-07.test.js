...
const sortStarWarsCharacters = (starWarsArr) =>
  starWarsArr.sort((a, b) => (+b.height > +a.height ? 1 : -1));
...
const howMuchPencil = (str) => {
  ...
  //   result.push( newStr.slice(i,str.length));
  // }
  let result = [];
  let strarr = [...str,' '];
  strarr.forEach((element,index) => { result.push( str.slice(index,strarr.length)); });
  return result;
};
...
below. Rather than taking the entire recipe,
you only want a list of the item names.

Write a function named listFoods that

takes in the recipe and

returns a new array of the food items

without any amount or units.
...
const listFoods = (recipe) => {
  ...
  for (const i in [...recipe.ingredients]) {
    const element =   (recipe.ingredients[i].slice(
      recipe.ingredients[i].indexOf(' ') + 1,
      recipe.ingredients[i].length ));
    result.push(element.slice(element.indexOf(' ') + 1,element.length) );
  }
  // elements.push(ingredients[i].slice(
  //   ingredients[i].indexOf(' ') + 1,
  //   ingredients[i].length ));

  //Last Index OF '   '
  // let str = ['1 medium-sized Gruffalo']
  // lastIndex = str[0].lastIndexOf(" ")
  // str[0] = str[0].slice(str[0].lastIndexOf(" "),str[0].length)
  // console.log(str[0]);
  // for (let i in ingredients) {
  //   result.push(
  //     ingredients[i].slice(
  //       ingredients[i].lastIndexOf(' ') + 1,
  //       ingredients[i].length
  //     )
  //   );
  // }
