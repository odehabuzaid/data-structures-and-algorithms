...
const addOne = (arr) => {
  let incrementedByOneArray = [];
  arr.forEach((element) => {
    incrementedByOneArray.push(element + 1);
  });
  return incrementedByOneArray;
};
...
const allUpperCase = (arr) => {
  let upperCasedStrArray = [];
  arr.forEach((element) => {
    upperCasedStrArray.push(element.toUpperCase());
  });
  return upperCasedStrArray;
};
...
const fizzbuzz = (arr) => {
  let outputArray = [];
  outputArray.length = arr.length;
  arr.forEach((element, index) => {
    outputBuilder(element, index);
  });
  function outputBuilder(element, index) {
    if (element % 3 === 0) {
      outputArray[index] = 'Fizz';
      if (element % 5 === 0) {
        outputArray[index] = 'Fizz Buzz';
      }
    } else {
      if (element % 5 === 0) {
