...
let results = [] ;
const addTwo = (arr) => {
  for (let i of arr){  results.push(i+2)  }
  return results
};
...
const typeNum = (arr) => arr.filter(el =>( (!isNaN(el))))
...
const containsAnd = arr => arr.filter(el => el.includes('and'))
...
const oddValues = arr => arr.filter( el => el % 2 !=0 )
...
getBaseStatGreaterThan that, 

given the snorlaxData, integer as input
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( el => el.baseStat > minBaseStat )
...
const getStatName = (arr, minBaseStat) => arr.filter( (el) => el.baseStat > minBaseStat ).map( (el) => el.stat.name )
...
const getCharactersWithoutChildren = arr => arr.filter(el => !el.children);
...
const evenOddNumericValues = arr => arr.filter(el => (typeof(el) == 'number')).map(el => ((el % 2 === 0) ? 'even' : 'odd'));
