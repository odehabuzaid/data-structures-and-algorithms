...
const updateAnimal = (arr, callback) => arr.map((el) => callback(el));
// const updateAnimal = (arr, callback) => {
//   let callBackOutPut = [];
//   // callBackOutPut = arr.map(el => callback(el)) ;
//   arr.filter(el => callBackOutPut.push(callback(el)));
...
const alphabetizeBetter = (arr) =>
  arr.sort((a, b) => lower(a).localeCompare(lower(b)));
...
const sortPeople = (arr) =>
  arr.sort((a, b) => lower(a.lastName).localeCompare(lower(b.lastName)));
...
const sortPeopleBetter = (arr) => {
  ...
  sortedArray.forEach((el, index) => {
    if (sortedArray[index + 1]) {
      if (el.lastName === sortedArray[index + 1].lastName) {
        sortedArray.sort((a, b) =>
          lower(a.firstName).localeCompare(lower(b.firstName))
        );
        if (el.firstName === sortedArray[index + 1].firstName) {
