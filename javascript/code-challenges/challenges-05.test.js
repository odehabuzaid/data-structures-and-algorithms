...
const addPurchases = (arr) => arr.reduce(((ac, cu) => ac + cu.purchasePrice), 0);
...
const countNumberOfElements = (arr) => arr.reduce((ac, cu) => cu ? ac + 1 : ac + 0, 0);
...
let names = [];
const returnNames = (arr) => arr.reduce((acc, cur) => {names.push(cur.name);
...
const reversedString = (str) => str.split('').reduce(((reverse, character) => character + reverse), '');
