...
const raisedToTheThird = (arr) => {
  let newArray = [];
  arr.forEach(element => { newArray.push(Math.pow(element, 3))});
  return newArray
};
...
For example, twoToThe([1,2,3]) returns [2,4,8] because 
2 ^ 1 = 2, 
2 ^ 2 = 4, and 
2 ^ 3 = 8.
------------------------------------------------------------------------------------------------ */

const forLoopTwoToThe = (arr) => arr.map(element => (Math.pow(2,element)))
...
const evenOdd = (arr) => arr.map(element => {
  if (isNaN(element)){return 'N/A'}else{
  switch (element % 2 ===0) {
    case true: 
      return 'even'
      case false: 
...
const extractStats = (arr) => arr.map(element=>({ name : element.stat.name,total : element.effort + element.baseStat}))
